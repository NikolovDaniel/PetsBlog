...
const ImageForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`https://kolombus-001-site1.htempurl.com/api/Images/${apiKey}`, formData);
      setShowModal(true);
      setPetImage(null);
    } catch (error) {
      ...
      <Row className='pet-form d-flex flex-column justify-content-center align-items-center mt-3 bg-dark'>
        <Col xs={12} md={6}>
          <Form className='d-flex flex-column mt-3' onSubmit={handleSubmit}>
            ...
            <Button className='custom-button mb-2 mt-2 justify-content-center fs-5' type='submit'>Submit</Button>
          </Form>
        </Col>
      </Row>
